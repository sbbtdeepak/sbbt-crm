...
import { getCompanyPublicData, getSocialLinks } from "@/app/dashboard/cms/actions";
import type { CompanyPublicData } from "@/app/dashboard/cms/types";
import { trackWhatsAppClick, trackOutboundClick } from "@/lib/analytics";
...
export default function Footer() {
  ...
  const renderSocialIcons = (className = "h-8 w-8") => (
    socialLinks.map((link) => {
      ...
      return (
        <a
          ...
          onClick={() => trackOutboundClick(link.url)}
          className={`flex items-center justify-center rounded-lg bg-slate-100 text-slate-500 transition hover:bg-indigo-100 hover:text-indigo-600 ${className}`}
        >
          {iconPath}
        </a>
      );
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 sm:py-10">
        ...
        <div className="hidden sm:grid sm:grid-cols-3 gap-8">
          ...
          <div>
            ...
            <div className="flex flex-col gap-1.5 text-sm text-slate-600">
              ...
              {company?.whatsapp && (
                <a href={`https://wa.me/${company.whatsapp.replace(/\D/g, '')}`} onClick={() => trackWhatsAppClick()} className="flex items-center gap-2 text-emerald-600 hover:text-emerald-700 transition">
        ...
        <div className="flex flex-col items-center text-center gap-5 sm:hidden">
          ...
          <div className="flex flex-col items-center gap-1">
            ...
            {address && <p className="text-xs text-slate-500 max-w-xs">{address}</p>}
          </div>
          ...
          <div>
            ...
            <div className="flex flex-col items-center gap-1.5 text-sm text-slate-600">
              ...
              {email && <p>{email}</p>}
              {company?.whatsapp && (
                <a href={`https://wa.me/${company.whatsapp.replace(/\D/g, '')}`} onClick={() => trackWhatsAppClick()} className="text-emerald-600 hover:text-emerald-700 transition font-medium">
                  WhatsApp
                </a>
              )}
            </div>
