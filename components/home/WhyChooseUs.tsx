const FEATURES = [
  ...
  },
  {
    ...
    icon: (
      <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5} aria-hidden="true">
        <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 013 9.749c0 5.592 3.824 10.128 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z" />
      </svg>
    ),
  },
  {
...
export default function WhyChooseUs() {
  return (
    <section className="bg-white py-6 sm:py-10 text-slate-900" aria-label="Why choose SBBT">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {/* Desktop Grid */}
        <div className="mt-4 hidden md:grid gap-3 sm:grid-cols-2 lg:grid-cols-3 auto-cols-fr">
          {FEATURES.map((feature) => (
            <div
              key={feature.title}
              className="group flex flex-col rounded-2xl border border-slate-100 bg-white p-4 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md hover:border-slate-200 h-full"
            >
              <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-indigo-50 text-indigo-600 transition-colors duration-300 group-hover:bg-indigo-600 group-hover:text-white flex-shrink-0">
                {feature.icon}
              </div>
              <h3 className="mt-3 text-sm font-semibold text-slate-900">
                {feature.title}
              </h3>
              <p className="mt-1 text-xs leading-4 text-slate-600 flex-1">
                {feature.description}
              </p>
            </div>
          ))}
        ...
        <div className="mt-4 md:hidden">
          <div className="flex gap-3 overflow-x-auto snap-x snap-mandatory pb-2 hide-scrollbar">
            {FEATURES.map((feature) => (
              <div
                key={feature.title}
                className="group snap-start flex-shrink-0 w-64 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-md flex flex-col h-full"
              >
                <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-indigo-50 text-indigo-600 transition-colors duration-300 group-hover:bg-indigo-600 group-hover:text-white flex-shrink-0">
                  {feature.icon}
                </div>
