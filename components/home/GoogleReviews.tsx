      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <p className="mt-4 text-lg leading-8 text-slate-600">
            Real reviews from Google. Verified by actual customers.
          </p>
        </div>
        ...
        <div className="mt-12 flex items-center justify-center gap-4">
          ...
          <div className="text-slate-600">64+ Reviews</div>
        </div>

        {/* Review Cards - Desktop Grid, Mobile Swipe Carousel */}
        <div className="mt-16">
          ...
          <div className="md:hidden">
            <div className="flex gap-4 overflow-x-auto snap-x snap-mandatory pb-4 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              {reviews.map((review, idx) => (
                <div
                  key={idx}
                  className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm hover:shadow-lg transition duration-300 snap-start flex-shrink-0 w-80"
                >
                  <div className="flex mb-3">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <svg
                        key={star}
                        className={`h-5 w-5 ${star <= review.rating ? "text-yellow-400" : "text-slate-300"}`}
                        fill="currentColor"
                        viewBox="0 0 20 20"
                      >
                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.069-3.292a1 1 0 00-.364-1.118L2.78 8.77c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                  ...
                  <p className="text-slate-700 mb-4">&ldquo;{review.review}&rdquo;</p>
          ...
          <div className="hidden md:grid md:grid-cols-3 gap-6">
            {reviews.map((review, idx) => (
              <div
                key={idx}
                className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm hover:shadow-lg transition duration-300"
              >
                <div className="flex mb-3">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <svg
        ...
        <div className="mt-12 flex flex-col sm:flex-row gap-4 justify-center">
          <a
            href="#"
            className="rounded-full bg-indigo-600 px-8 py-3 font-semibold text-white shadow-lg hover:bg-indigo-500 transition"
          >
            View All Reviews
          </a>
          <a
            href="#"
            className="rounded-full border border-slate-300 bg-white px-8 py-3 font-semibold text-slate-900 hover:bg-slate-50 transition"
          >
            Write a Review
          </a>
        </div>
